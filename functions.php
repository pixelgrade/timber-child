<?php
/**
 * Timber Child functions and definitions
 *
...
/**
 * Setup Timber Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function timber_child_theme_setup() {
	load_child_theme_textdomain( 'timber-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'timber_child_theme_setup' );
...
function timber_child_enqueue_styles() {

	// Here we are adding the child style.css while still retaining 
	// all of the parents assets (style.css, JS files, etc)
	// 
	wp_enqueue_style( 'timber-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('timber-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);
}

add_action( 'wp_enqueue_scripts', 'timber_child_enqueue_styles' );
...
function timber_child_overwrite_files() {
	...
	// See the parent's function.php -> the timber_scripts_styles() function
	// for details like resources names

		wp_dequeue_script( 'timber-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'timber-child-scripts',
...
// Load the files from the function mentioned above:
	...
	add_action( 'wp_enqueue_scripts', 'timber_child_overwrite_files', 11 );
